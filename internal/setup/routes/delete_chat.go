...
func DeleteChat(server *http.ServeMux, db *mongo.Database) {
	server.Handle("DELETE /chat", middlewares.VerifyAccessToken(adapters.AdaptRoute(factory.MakeDeleteChatController(db))))
}
